...
- Serialized orphaned Parquet cleanup with active workspace mutations so a
  newly opened store cannot delete another process's in-progress write.
...
- Advance the product release-candidate metadata without functional changes from RC1.
...
- Run shared preprocessing inside a `by_source` native DAG-ML branch, retaining
  the existing legacy fallback for unsupported branch semantics.
