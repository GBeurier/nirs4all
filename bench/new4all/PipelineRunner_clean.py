...
from PipelineHistory import PipelineHistory, StepExecution


class PipelineRunner:
    """Clean pipeline execution engine with builder delegation and history tracking"""
    ...
    def run_pipeline(self, pipeline_config: Union[Dict, List, PipelineConfig], dataset: SpectraDataset) -> Tuple[SpectraDataset, PipelineHistory]:
        ...
        self.history.start_pipeline_execution(pipeline_config)
        ...
        if isinstance(pipeline_config, PipelineConfig):
            ...
            raise ValueError(f"Invalid pipeline config type: {type(pipeline_config)}")        # Execute pipeline steps
        try:
            for step in steps:
                self._run_step(step, dataset)

            # Complete pipeline execution
            self.history.complete_pipeline_execution()
            print("✅ Pipeline completed successfully")

        except Exception as e:
            self.history.fail_pipeline_execution(str(e))
    ...
    def _run_feature_augmentation_sequential(self, augmenters: List[Any], dataset: SpectraDataset, train_view, prefix: str):
        ...
        for i, augmenter in enumerate(augmenters):
            ...
            try:
                ...
                print(f"{prefix}  ✅ Augmenter {i+1} completed")            except Exception as e:
