...
### C.1 Native-coverage boundary — `EXPECTED_FALLBACK` (7)
...
| by-source / per-source / source-concat multi-source | `multi_source_by_source_branch_distinct_preproc`, `multi_source_per_source_models_stacking`, `multi_source_sources_concat_then_rf` |
...
| Runnable | **87** | 95 − 8 |
| → fall back to legacy (`EXPECTED_FALLBACK`) | **7** | boundary-asserted, no parity claim — **target → 0 (LOCK-DROP D1, L5)** |
| → run native on dag-ml | **80** | full parity asserted |
...
> strict-xfail (11), `EXPECTED_FALLBACK` (7), and
