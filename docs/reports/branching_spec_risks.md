...
|------|------------|--------|------------|
| **Execution Risks** | | | |
| Context leakage between branches | Medium | High | Deep copy contexts; thorough isolation tests |
| Memory growth with many branches | Medium | Medium | Lazy context restoration; shared dataset reference |
| Complex nested branch scenarios | Medium | Medium | Start simple; iterative complexity; limit nesting depth |
| Outlier detection inconsistency | Medium | Medium | Fixed random seeds; deterministic algorithms |
| **Serialization Risks** | | | |
| Artifact naming conflicts | Low | Medium | Include branch_id in artifact names; use content-hash for storage |
| Manifest schema migration | Low | Medium | Backward-compatible defaults (branch_id=None for legacy) |
| Missing branch artifacts in predict mode | Medium | High | Clear error messages; validate manifest on load |
| Branch count mismatch (train vs predict) | Low | High | Validate branch structure matches manifest before prediction |
| Corrupt/incomplete manifests | Low | High | Atomic writes; validation on load |
| Nested branch path serialization | Low | Medium | Store as list, reconstruct as tuple on load |
| **Reproducibility Risks** | | | |
| Non-deterministic predictions after reload | Low | Critical | Comprehensive roundtrip tests; fixed random seeds |
| Floating-point precision loss | Low | Medium | Verify precision in tests; use high-precision serialization |
| Outlier mask inconsistency | Medium | High | Store masks in artifacts; verify on reload |
| **Integration Risks** | | | |
| Breaking existing pipelines | Low | High | New keyword, no changes to existing syntax |
| Prediction mode complexity | Medium | Medium | Phase 5 dedicated to prediction testing |
| Generator expansion inside branches | Medium | Medium | Comprehensive test coverage for nested generators |
| Visualization performance with many branches | Low | Low | Pagination; limit displayed branches |
...
### Functional Criteria
1. ✅ `branch` syntax works with explicit branch lists
2. ✅ Post-branch steps execute on each branch independently
3. ✅ In-branch Y processing is isolated per branch
4. ✅ In-branch models train correctly
5. ✅ Generators (`_or_`, `_range_`) work inside branch specs
6. ✅ Named branches work with dictionary syntax

### Nested Branch Criteria
7. ✅ Nested branches create Cartesian product of configurations
8. ✅ Branch path encoding correctly identifies nested branches
9. ✅ Nested branch artifacts are uniquely named with full path
10. ✅ Prediction filtering works with partial path matching

### Data & Context Criteria
11. ✅ Predictions include `branch_id`, `branch_name`, and `branch_path`
12. ✅ CV folds are shared across branches (single split)
13. ✅ Dataset is loaded once (not N times for N branches)
14. ✅ Each branch has independent X processing chain
15. ✅ Each branch has independent Y processing state

### Outlier Excluder Criteria
16. ✅ Outlier excluder branching syntax works
17. ✅ Multiple outlier strategies can be compared
18. ✅ Exclusion applies only to training data
19. ✅ `excluded` field is included in prediction metadata
20. ✅ Baseline branch (no exclusion) is supported

### Serialization & Persistence Criteria
21. ✅ Artifacts are saved with branch-aware names (`{name}_b{id}_{op}`)
22. ✅ Manifest includes `branch_id`, `branch_name`, and `branch_path` for each artifact
23. ✅ `pipeline.json` preserves branch structure for replay
24. ✅ `BinaryLoader` supports loading by (step, branch_id) and (step, branch_path)
25. ✅ Prediction mode correctly replays branched pipelines
26. ✅ Can predict using a specific branch's model

### Reproducibility Criteria
27. ✅ Roundtrip test passes: train → save → load → predict with identical results
28. ✅ Predictions match to floating-point precision (< 1e-12 difference)
29. ✅ Transformer states are correctly restored after reload
30. ✅ Nested branch roundtrip produces identical results
31. ✅ Outlier exclusion masks are consistent after reload

### Visualization Criteria
32. ✅ Heatmaps with branch grouping work correctly
33. ✅ Box plots and bar charts compare branches
34. ✅ Branch DAG diagram renders correctly
35. ✅ Summary statistics table includes branch breakdown
36. ✅ Nested branch visualization shows hierarchical structure
37. ✅ HTML report includes branch analysis section

### Compatibility Criteria
38. ✅ Full backward compatibility with existing pipelines
39. ✅ Legacy manifests (no branch_id) work correctly
40. ✅ Visualization tools support branch comparison
