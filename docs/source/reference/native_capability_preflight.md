...
| Retrain support depends on mode, engine, plugin selector, session state, and bundle contents. | `retrain_preflight(...).to_dict()` is side-effect-free; native full retrain then validates exactly one bounded `train_pipeline.json` member. | Full retrain uses native DAG-ML. Transfer uses the explicit built-in `plugin="nirs4all-python-library"` adapter. Finetune and unknown plugins refuse before inputs are touched. |
