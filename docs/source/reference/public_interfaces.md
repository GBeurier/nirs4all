...
| `nirs4all.load_session(...)` | Load an exported bundle for prediction; select `engine="native"` for a validated Archive V2 Methods PREDICT session | Bundle path; native archives require `engine="native"` | `Session` or `NativeArchiveSession` |
