...
| --- | --- | --- |
| `run(..., engine="native")` | Supported non-terminal subset | Explicit raw mapping `{"X", "y", "sample_ids"}`, portable Methods pipeline, native refit, no `terminal_predict` cohort, and no legacy workspace/cache/project/result path. Its in-memory result starts with the local strict-execution observation described above. Nested stacking requires a partitioned outer CV (for example `KFold`): repeated or overlapping validation such as `ShuffleSplit` is refused before execution because it cannot provide one outer-OOF row per sample. Unsupported shapes stop before a legacy run. |
| `run(..., terminal_predict={"X", "sample_ids"}, engine="native")` | Supported strict terminal form | Requires explicit raw training `{"X", "y", "sample_ids"}` and target-free terminal cohorts, raw numeric arrays, one unshuffled `KFold`, one default-shape `PLSRegression`, and native refit. It uses only the callback-free DAG-ML CV→REFIT→terminal-PREDICT facade; groups, metadata, transforms, HPO, calibration, external OOF state, sessions, cache/project/result-path state, and legacy runners are refused before execution. The returned ordinary terminal prediction is non-attesting; its opaque frozen receipt is process-local and Archive V2 never archives, reloads, or forges that terminal receipt. |
