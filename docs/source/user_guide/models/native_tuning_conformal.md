...
| --- | --- | --- | --- |
| `run(engine="dag-ml")` | Selects the DAG-ML execution backend for the current run. | `RunResult` metadata and backend traces. | It is not an optimizer selector; `run(engine="dual")` is a separate bounded parity oracle. |
