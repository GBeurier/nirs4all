...
runner.run(dataset, pipeline, dest="workspace/stacking")
```
...
pipeline = [
    ...
    Branch(
        branches=[
            ...
            [DerivativeTransform(order=1), PLSRegression(n_components=5)],  # D1
            [DerivativeTransform(order=2), PLSRegression(n_components=5)],  # D2
        ],
...
- [Example: Q_meta_stacking.py](../../examples/Q_meta_stacking.py) - Comprehensive examples
- [Branching Guide](./branching.md) - Using branches with stacking
- [Pipeline Guide](./pipeline.md) - General pipeline usage
