...
class PipelineHistory:
    ...
    def start_execution(self, pipeline_config: Union[Dict, List], metadata: Optional[Dict] = None) -> str:
        ...
        if hasattr(pipeline_config, '__dict__'):
            ...
            config_dict = {"pipeline": pipeline_config}
        else:
    ...
    def save_pipeline_bundle(self, filepath: Union[str, Path], pipeline_data: Dict[str, Any]):
        """
        Save complete pipeline bundle with execution history
        
        Args:
            filepath: Path to save bundle (determines format from extension)
            pipeline_data: Pipeline data from runner.get_fitted_pipeline()
        """
        filepath = Path(filepath)
        
        if filepath.suffix.lower() == '.zip':
            self._save_zip_bundle(filepath, pipeline_data)
        elif filepath.suffix.lower() == '.pkl':
            self._save_pickle_bundle(filepath, pipeline_data)
        elif filepath.suffix.lower() == '.json':
            self._save_json_bundle(filepath, pipeline_data)
        else:
            # Default to zip
