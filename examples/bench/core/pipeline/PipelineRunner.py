...
class PipelineRunner:
    ...
    def run(self, config: Union[str, Dict[str, Any]], dataset: SpectraDataset) -> SpectraDataset:
        """
        Execute the pipeline based on the provided configuration and dataset
        """
        ...
        if isinstance(config, str):
            if config.endswith(".json"):
                with open(config, 'r', encoding='utf-8') as f:
                    pipeline_config = json.load(f)["pipeline"]
            ...
            raise ValueError("Config must be a dictionary or a path to a .json or .yaml file")
        ...
        try:
            ...
            print(f"❌ Unexpected error during pipeline execution: {str(e)}")
            if not self.continue_on_error:
                raise
    ...
    def _run_sub_pipeline(self, step_list: List[Any], dataset: SpectraDataset, prefix: str):
        ...
        print(f"📋 Sub-pipeline with {len(step_list)} steps")
    ...
    def _run_splitting(self, splitter: Any, dataset: SpectraDataset, prefix: str):
        ...
        print(f"✂️  Splitting: {splitter.__class__.__name__}")
        ...
        if isinstance(splitter, BaseCrossValidator):
            ...
            if len(X) == 0:
                print(f"   ⚠️  No data found for splitting")
                return
    ...
    def _run_clustering(self, clusterer: Any, dataset: SpectraDataset, prefix: str):
        ...
        print(f"🎯 Clustering: {clusterer.__class__.__name__ if hasattr(clusterer, '__class__') else str(clusterer)}")
        ...
        print(f"   🏷️  Updated context with group filtering")
    ...
    def _run_sample_augmentation(self, augmenters: List[Any], dataset: SpectraDataset, prefix: str):
        ...
        print(f"🔄 Sample augmentation with {len(augmenters)} augmenter(s)")

        for i, augmenter in enumerate(augmenters):
            if augmenter is None:
                print(f"   {i+1}. Identity (no augmentation)")
                continue

            print(f"   {i+1}. {augmenter.__class__.__name__ if hasattr(augmenter, '__class__') else str(augmenter)}")
            ...
            if hasattr(augmenter, 'transform'):
                ...
                print(f"     ⚠️  Augmenter does not support transform")
    ...
    def _run_feature_augmentation(self, augmenters: List[Any], dataset: SpectraDataset, prefix: str):
        ...
        print(f"🔧 Feature augmentation with {len(augmenters)} augmenter(s)")
        ...
        for i, augmenter in enumerate(augmenters):
            ...
            if isinstance(augmenter, list):
                print(f"   {i+1}. Pipeline of {len(augmenter)} transformers")
    ...
    def _run_dispatch_parallel(self, branches: List[Any], dataset: SpectraDataset, prefix: str):
        ...
        def execute_branch(branch_data):
            ...
            branch_prefix = f"  Branch {branch_idx+1}: "
    ...
    def _merge_branch_results(self, results: Dict[int, Tuple], dataset: SpectraDataset, prefix: str):
        ...
        print(f"   🔄 Merging {len(results)} branch results")
        ...
        print(f"   ✅ Merged predictions from {len(merged_predictions)} models")

    def _log_step_start(self, step: Any, prefix: str):
        """Log step execution start"""
        step_desc = self._get_step_description(step)
        print(f"🔄 Step {self.current_step}: {step_desc}")
    ...
    def _log_step_error(self, step: Any, error: Exception, prefix: str):
        ...
        step_desc = self._get_step_description(step)
        print(f"❌ Step {self.current_step} failed: {step_desc} - {str(error)}")
