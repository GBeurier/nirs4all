...
def example_pipeline_with_target_management():
    ...
    pipeline.add_operation(TransformationOperation(StandardScaler()))
    pipeline.add_operation(ClusteringOperation(n_clusters=3, store_centroids=True))
    pipeline.add_operation(ModelOperation(RandomForestClassifier(n_estimators=50)))
