...
class PipelineRunner:
    ...
    def _handle_sklearn_estimator(self, step, data, prefix=""):
    ...
    def _handle_transformer(self, step, data, prefix=""):
        ...
        test_features = data._select(set="test")
        X_test = data.X(set="test", pad=True) if len(test_features) > 0 else None
        ...
        processing_hash = hashlib.md5(
            f"{step.__class__.__name__}_raw".encode()).hexdigest()[:8]
        data.set_tag("processing", processing_hash)

    def _handle_clustering(self, step, data, prefix=""):
    ...
    def _handle_sample_augmentation(self, transforms, data, prefix=""):
    ...
    def _handle_feature_augmentation(self, transforms, data, prefix=""):
        ...
        for transform in transforms:
            ...
            # Update processing hash
            feat_processing_hash = hashlib.md5(
                f"feat_{transform.__class__.__name__}_raw".encode()).hexdigest()[:8]
            data.set_tag("processing", feat_processing_hash)

    def _handle_data_splitter(self, step, data, prefix=""):
    ...
    def run(self, step, data, context, prefix=""):
        ...
        if isinstance(step, dict) and len(step) == 1:
            ...
            if key == "cluster":
                self._handle_clustering(spec, data, prefix=prefix + "  ")
        ...
        if isinstance(step, dict) and len(step) > 1:
            if "model" in step:
                ...
                self._handle_clustering(step, data, prefix=prefix + "  ")
            else:
        ...
        if hasattr(step, "transform") and issubclass(step.__class__, TransformerMixin):
            self._handle_transformer(step, data, prefix=prefix)
        elif hasattr(step, "split"):
            self._handle_data_splitter(step, data, prefix=prefix)
        elif hasattr(step, "fit") and issubclass(step.__class__, ClusterMixin):
            self._handle_clustering(step, data, prefix=prefix)
        elif issubclass(step.__class__, BaseEstimator):
            self._handle_sklearn_estimator(step, data, prefix=prefix)
        else:
            p(f"Unknown step: {step}")
    ...
    def run_model(self, step, data, context, prefix=""):
        ...
        if hasattr(model, 'fit') and issubclass(model.__class__, BaseEstimator):
            self._handle_sklearn_estimator(model, data, prefix=prefix + "  ")
        else:
