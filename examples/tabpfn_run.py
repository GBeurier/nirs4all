...
def train_tabpfn(task_type, model_variants=None, aug=0.0, cv=False, transf=None, mode="normal"):  # rf, finetune
    ...
    if mode == "finetune":
        if task_type == 'classification':
            TabPFN_class = TunedTabPFNClassifier(device='cuda')
        else:
            TabPFN_class = TunedTabPFNRegressor(device='cuda')

    elif mode == "rf":
        if task_type == 'classification':
            TabPFN_class = RandomForestTabPFNClassifier(
                tabpfn=TabPFNClassifier(device='cuda', eval_metric="balanced_accuracy", n_estimators=12),
                n_estimators=100,
                max_depth=12,
            )
        else:
...
predictions, predictions_per_dataset = train_tabpfn(
    ...
    model_variants=['real', 'variant'],
    transf=[SpectralLatentFeatures(
        use_pca=True,
        n_pca=120,
        pca_whiten=True,
        ...
        nmf_max_iter=200,
        #####
        use_band_areas=False,
        n_area_bands=10,
        area_include_ratios=True,
        #####
        use_peaks=False,
        n_peaks=1,
        #####
