...
class NativeMethodsRefitResult:
    ...
    def from_package_json(
        ...
        *,
        ...
        decoder: Callable[[Any, Any], np.ndarray] | None = None,
        ...
        if methods_library_path is None:
            resolved_methods_library_path = None
        elif isinstance(methods_library_path, str) and methods_library_path:
            # Package parsing is a pure contract check.  Defer filesystem and
            # ABI validation until `predict`, so callers can inspect or move a
            # package on hosts that do not yet have libn4m installed.
            resolved_methods_library_path = methods_library_path
        else:
            raise TypeError("methods_library_path must be a non-empty string or None")
        return cls(
            package,
            methods_library_path=resolved_methods_library_path,
            dagml_module=dagml_module,
            decoder=_default_decoder() if decoder is None else decoder,
        )
    ...
    def predict(
        ...
        *,
        ...
        replay = RawArrayMethodsPortableRefitReplayCompiler(
            self._package,
            methods_library_path=resolve_methods_library_path(self._methods_library_path),
...
def _default_decoder() -> Callable[[Any, Any], np.ndarray]:
    """Resolve the shared strict output decoder without an import cycle."""

    from .native_training import _decode_raw_methods_prediction

    return cast(Callable[[Any, Any], np.ndarray], _decode_raw_methods_prediction)
