...
_DUAL_MAX_LEDGER_BYTES = 256 * 1024
_DUAL_MAX_TOLERANCE_BANDS = 128
_DUAL_MAX_SAMPLES = 4_096
_DUAL_MAX_FEATURES = 2_048
_DUAL_MAX_FOLDS = 20
_DUAL_MAX_COMPONENTS = 64
_DUAL_MAX_ITERATIONS = 1_000
_DUAL_MAX_MISMATCHES = 256
...
def _resolve_dual_tolerances() -> dict[str, dict[str, Any]]:
    ...
    try:
        ledger_bytes = importlib.resources.files("nirs4all").joinpath(_DUAL_COMPATIBILITY_LEDGER_RESOURCE).read_bytes()
        if len(ledger_bytes) > _DUAL_MAX_LEDGER_BYTES:
            raise DualRunUnsupported(
                f"engine='dual' refuses a compatibility ledger larger than {_DUAL_MAX_LEDGER_BYTES} bytes"
            )
        ledger = json.loads(ledger_bytes.decode("utf-8"))
    except (FileNotFoundError, ModuleNotFoundError, OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
    ...
    if len(ledger["tolerance_bands"]) > _DUAL_MAX_TOLERANCE_BANDS:
        raise DualRunUnsupported(
            f"engine='dual' refuses more than {_DUAL_MAX_TOLERANCE_BANDS} compatibility-ledger tolerance bands"
        )
...
def _require_dual_supported_request(
    ...
    *,
    ...
    if X.shape[0] > _DUAL_MAX_SAMPLES or X.shape[1] > _DUAL_MAX_FEATURES:
        raise DualRunUnsupported(
            f"engine='dual' bounds its oracle to at most {_DUAL_MAX_SAMPLES} samples and {_DUAL_MAX_FEATURES} features"
        )
    if (
    ...
    if type(n_splits) is not int or not 2 <= n_splits <= min(_DUAL_MAX_FOLDS, X.shape[0]):
        raise DualRunUnsupported(
            f"engine='dual' requires 2..{min(_DUAL_MAX_FOLDS, X.shape[0])} concrete KFold splits"
        )
    model = model_step["model"]
    if type(model.n_components) is not int or not 1 <= model.n_components <= min(_DUAL_MAX_COMPONENTS, X.shape[1]):
        raise DualRunUnsupported(
            f"engine='dual' requires 1..{min(_DUAL_MAX_COMPONENTS, X.shape[1])} PLS components"
        )
    if type(model.max_iter) is not int or not 1 <= model.max_iter <= _DUAL_MAX_ITERATIONS:
        raise DualRunUnsupported(
            f"engine='dual' bounds PLS max_iter to 1..{_DUAL_MAX_ITERATIONS}"
        )
    ...
    if project is not None or session is not None or cache is not None or results_path is not None or allow_fallback or runner_kwargs:
        raise DualRunUnsupported("engine='dual' does not support project, session, cache, results_path, allow_fallback, or runner kwargs")
...
def _append_dual_mismatch(mismatches: list[dict[str, Any]], mismatch: dict[str, Any]) -> None:
    """Retain a bounded mismatch ledger while preserving truncation evidence."""

    if len(mismatches) < _DUAL_MAX_MISMATCHES:
        mismatches.append(mismatch)
    elif len(mismatches) == _DUAL_MAX_MISMATCHES:
        mismatches.append(
            {
                "field": "mismatches",
                "reason": "truncated",
                "retained": _DUAL_MAX_MISMATCHES,
            }
        )
...
def run(
    ...
    **runner_kwargs: Any,
    ...
    Args:
        ...
        allow_fallback: Frozen compatibility parameter. ``False`` is required for native engines;
            ``True`` is rejected before input inspection. Select ``engine="legacy"`` explicitly to use
            the rollback lane (where this parameter has no effect).
    ...
    Raises:
        ...
        DualRunUnsupported: If the explicit dual request, native capability, packaged ledger, or
            concrete OOF evidence is outside the bounded oracle contract.
        DualRunMismatchError: If the native and explicit temporary legacy legs differ outside the
            ledger tolerances.
    ...
    def _run_legacy(
        *,
        ...
        local_runner_kwargs: Mapping[str, Any] | None = None,
    ) -> RunResult:
        ...
        effective_runner_kwargs = dict(runner_kwargs if local_runner_kwargs is None else local_runner_kwargs)
    ...
    if selected_engine == "dual":
        ...
        try:
            ...
            raise DualRunUnsupported("engine='dual' could not isolate inputs for independent native/legacy execution") from error
        ...
        try:
            ...
            raise DualRunUnsupported(
                "engine='dual' requires a supported native DAG-ML counterpart; no legacy fallback was run and no legacy oracle was invoked"
            ) from error
        native_seconds = time.perf_counter() - native_started

        # Refuse incomplete native evidence before allocating the temporary
        # legacy workspace or invoking the rollback implementation.
        ...
        # The native result remains the product output. The explicit legacy
        # leg is comparison-only and its workspace has already been removed.
