...
class SpectraSet:
    ...
    def X(
        ...
        *,
        ...
        include_sources: Union[bool, str, Sequence[Any]] = True,
        ...
        all_spectral_vars = [str(k) for k in ds.data_vars if isinstance(k, str) and k.startswith("spectra_")]
        vars_src: list[str] = []

        if isinstance(include_sources, str):
            source_name = f"spectra_{include_sources}"
            if source_name in all_spectral_vars:
                vars_src = [source_name]
            # If source_name is not in all_spectral_vars, vars_src remains empty, handled later
        elif include_sources is False:  # Explicitly False
            if all_spectral_vars:  # Check if there's at least one source
                vars_src = all_spectral_vars[:1]  # pick first
        elif isinstance(include_sources, Sequence):  # Sequence of source names
            vars_src = [f"spectra_{s}" for s in include_sources if f"spectra_{s}" in all_spectral_vars]
        elif include_sources is True:  # Explicitly True
            vars_src = all_spectral_vars  # include all sources
        
        # If, after filtering, only one source is selected, return its data directly
        if len(vars_src) == 1:
            if vars_src[0] in ds:
                return ds[vars_src[0]].values
            else:  # Should ideally not happen
                num_obs = len(ds.obs)
                return np.empty((num_obs, 0))

        # Collect arrays for concatenation (multiple sources or no specific single source)
        arrs = []
        for var in vars_src:
            if var in ds:  # Ensure variable exists in the current dataset view
                da = ds[var]
                arrs.append(da.values)
        
        if not arrs:  # Handle case where no sources matched or were selected
            num_obs = len(ds.obs)
            return np.empty((num_obs, 0))

        # horizontal concatenation for multiple sources
