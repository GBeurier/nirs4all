...
def _duplication_branch_bodies(step: Any) -> list[list[Any]] | None:
    """Return ordered duplication bodies for list or named-dict syntax, else ``None``.

    Legacy treats a list branch and a dict branch without a ``by_*`` criterion as duplication: every
    branch sees the same samples. Dict insertion order is observable by legacy merge configuration, so
    preserve it while dropping only branch-level configuration/internal keys.
    """
    if not isinstance(step, dict):
        return None
...
def _is_duplication_branch_step(step: Any) -> bool:
    """True for a legacy duplication branch in list or named-dict syntax."""
...
def _simple_duplication_merge_mode(step: Any) -> str | None:
    """Return a simple duplication feature merge handled by this native slice."""
    if not isinstance(step, dict) or "merge" not in step:
        return None
    return "features" if step["merge"] == "features" else None


def _model_step_is_plain_estimator(model_step: dict[str, Any]) -> bool:
    """Whether the downstream model is a bare estimator whose options we lower exactly."""
    ...
    if model is None or not (hasattr(model, "fit") and hasattr(model, "predict")):
        return False
    return not any(
        key not in _RESERVED_STEP_KEYS or is_param_generator_spec(value)
...
def _detect_duplication_branch(pipeline: list[Any]) -> tuple[list[list[Any]], str] | None:
    """Detect a handled duplication-fusion or feature-merge shape, else ``None``.

    Admits either a duplication branch with a fusion merge and one model per branch, or a feature-only
    duplication branch followed by ``{"merge": "features"}`` and one plain downstream estimator.
    ANY deviation returns ``None`` so the bridge's
    ...
    * a separation (``by_*`` dict-form) branch — handled elsewhere;
    ...
    * structured merge configuration, MetaModel, or branch-local model for a feature merge.
    """
