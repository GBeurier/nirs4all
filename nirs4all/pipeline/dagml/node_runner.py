...
class _SourceConcatEstimator:
    ...
    This represents legacy ``by_source`` preprocessing + ``merge: concat``:
    every source has its own cloned preprocessing chain fit on that source's
    fold-train block; validation/test blocks are transformed by those fitted
    chains and concatenated in source-layout order before the downstream model.
    """

    def __init__(self, model: Any, source_chain_templates: list[list[Any]], preserve_legacy_sources_after_merge: bool = False) -> None:
        self._model = model
    ...
    def fit(self, blocks: list[np.ndarray], y: Any) -> _SourceConcatEstimator:
        ...
        if len(blocks) != len(self._source_chain_templates):
            raise ValueError(
                f"by_source concat received {len(blocks)} blocks for {len(self._source_chain_templates)} source chain(s)"
            )
        self._source_chains = [[clone(step) for step in chain] for chain in self._source_chain_templates]
    ...
    if phase == "PREDICT":
        ...
        if source_concat:
            estimator = _SourceConcatEstimator(
                model,
                source_chains or [],
                preserve_legacy_sources_after_merge=_source_concat_preserve_legacy_sources(node_lookup(node_id)),
            )
        elif multi_block:
