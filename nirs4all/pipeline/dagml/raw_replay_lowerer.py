...
class RawArrayMethodsPortableRefitReplayCompiler:
    ...
    Package V3 is intentionally not coerced into the V2 predictor shape: the
    child has a fresh refit outcome and Core validates it through its dedicated
    replay entry point.  This lowerer only derives the current raw-cohort
    envelopes and numeric provider inputs; it does not inspect or hydrate the
    N4MM bytes.
    """
...
def validate_native_methods_refit_package_v3(package: Any) -> dict[str, Any]:
    ...
    DAG-ML remains responsible for strict TCV1 package validation when the
    replay call executes.  The Python boundary checks only the fields it needs
    to lower current data safely and refuses V2/host-sidecar substitution.
    """

    document = _package_document(package)
    if document.get("schema_version") != 3:
        raise RawArrayMethodsReplayError(
            "raw-array Methods refit replay requires Package V3"
        )
...
def _refit_v3_requirements(plan: Mapping[str, Any]) -> dict[str, dict[str, str]]:
    ...
    for node in node_plans.values():
        ...
        for binding in bindings:
            ...
            if key in requirements:
                raise RawArrayMethodsReplayError(
                    "Package V3 repeats a data requirement key"
                )
            requirements[key] = {
                "schema_fingerprint": schema,
