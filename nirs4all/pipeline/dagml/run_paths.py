...
class DuplicationBranchMergeTransformer(TransformerMixin, BaseEstimator):
    """Fold-local sklearn transformer for legacy duplication ``merge="features"``."""
    ...
    def fit(self, X: Any, y: Any = None) -> DuplicationBranchMergeTransformer:
        if self.merge_mode != "features":
            raise ValueError(f"unsupported duplication branch merge mode {self.merge_mode!r}")
    ...
    def transform(self, X: Any) -> np.ndarray:
        ...
        return np.concatenate(
            [self._transform_branch_features(X_arr, fitted["transforms"]) for fitted in self._fitted_branches],
            axis=1,
        )
    ...
    def _fit_branch(self, X: np.ndarray, y: Any, branch: dict[str, Any]) -> dict[str, Any]:
        ...
        for spec in branch["steps"]:
            if spec["kind"] != "transform":
                raise ValueError(f"unsupported branch feature merge step kind {spec['kind']!r}")
...
def _branch_merge_transformer_step(branches: list[list[Any]], merge_mode: str) -> DuplicationBranchMergeTransformer:
    """Build the importable fold-local transformer used for duplication feature merge."""
    from nirs4all.pipeline.dagml_bridge import _json_safe_params, _qualname

    if merge_mode != "features":
        raise DagMlUnsupported(
            "engine='dag-ml' supports duplication branch merge only for merge='features' in this slice"
        )
        ...
        for step in _supported_body_steps([part for part in branch if not hasattr(part, "split")]):
            ...
            if isinstance(step, dict):
                raise DagMlUnsupported(
                    ...
                    f"structured branch step dictionaries such as {sorted(step)} stay on the legacy path"
                )
            lowered_steps.append(
                {"kind": "transform", "class": _qualname(step), "params": _json_safe_params(step)}
            )
...
def _run_duplication_branch_feature_merge(
    pipeline: list[Any],
    ...
    """Run a legacy duplication feature merge with fold-local branch preprocessing."""
    splitter = next((step for step in pipeline if hasattr(step, "split")), None)
    ...
    if splitter is None or model_step is None:
        raise DagMlUnsupported(
            "engine='dag-ml' requires a cross-validator and downstream model for duplication merge=features"
        )
    return _run_concrete(
        [splitter, _branch_merge_transformer_step(branches, merge_mode), model_step],
        spectro,
        dataset_arg,
        cli,
        venv_python,
