...
class PipelineRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            n_jobs: Number of parallel workers for pipeline variants (sweeps).
                - 1: Sequential execution (default)
                - -1: Use all available CPU cores
                - N > 1: Use N parallel workers
                Note: This parallelizes different pipeline variants generated by
                _cartesian_, _or_, etc. Branch parallelization (branch keyword)
                is controlled separately via branch-level parallel config.
            report_naming: Naming convention for metrics in reports ("nirs", "ml", or "auto").
                Defaults to "nirs" (chemometrics terminology).
    ...
    def run(
        ...
        Args:
            ...
            refit: Refit configuration.
                ...
                - ``dict``: Single criterion, e.g. ``{"top_k": 3}``.
                - ``list[dict]``: Multiple criteria for union selection.
