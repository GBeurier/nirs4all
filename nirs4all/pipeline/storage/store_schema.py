...
CREATE UNIQUE INDEX IF NOT EXISTS idx_predictions_natural_key ON predictions(pipeline_id, chain_id, fold_id, partition, model_name);
...
def _migrate_schema(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    # Migration: ensure chain_id and model_name columns exist before unique index
    ...
    # Migration: deduplicate predictions before unique index creation
    existing_indexes = {
        row[0]
        for row in conn.execute(
    ...
    if "idx_predictions_natural_key" not in existing_indexes:
        dup_count = conn.execute(
            "SELECT COUNT(*) FROM ("
            "  SELECT pipeline_id, chain_id, fold_id, partition, model_name "
            "  FROM predictions "
            "  GROUP BY pipeline_id, chain_id, fold_id, partition, model_name "
            "  HAVING COUNT(*) > 1"
            ")"
        ).fetchone()[0]
        if dup_count > 0:
            ...
            conn.execute(
                ...
                "      ROW_NUMBER() OVER ("
                "        PARTITION BY pipeline_id, chain_id, fold_id, partition, model_name "
