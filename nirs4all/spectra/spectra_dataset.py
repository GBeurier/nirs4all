...
from nirs4all.spectra.spectra_targets import SpectraTargets
from nirs4all.spectra.spectra_predictions import SpectraPredictions
from nirs4all.spectra.spectra_loader import load_data_from_config

if TYPE_CHECKING:
    from nirs4all.spectra.dataset_view import DatasetView
...
class SpectraDataset:
    ...
    INDICES_SCHEMA = {
        ...
        "processing": pl.Series([], dtype=pl.Utf8),    # Processing level/stage        # "replication": pl.Series([], dtype=pl.Int64),  # Replication index (if applicable)
    }
    ...
    def __init__(self, float64: bool = True, task_type: str = "auto"):
        ...
        self.indices = pl.DataFrame(self.INDICES_SCHEMA)  # Indices DataFrame with proper schema        # Target management
        self.target_manager = SpectraTargets(task_type=task_type)

        # Results and folds management
        self.predictions = SpectraPredictions()
    ...
    def copy(self):
        ...
        new_dataset.predictions = copy.deepcopy(self.predictions)
    ...
    def sample_augmentation(self,
        ...
        for copy_idx in range(n_copies):
            ...
            # Create new row IDs
            copy_row_ids = list(range(self._next_row, self._next_row + len(original_sample_ids)))            # Copy features for all sources
            if self.features is not None:
                original_features = []
    ...
    def add_predictions(self,
        ...
        self.predictions.add_predictions(
            sample_ids=sample_ids,
            ...
            fold=fold,
            seed=seed,
            branch=branch, stack_index=stack_index,
    ...
    def get_fold_predictions(self,
        ...
        return self.predictions.get_fold_predictions(
            model_name=model_name,
    ...
    def get_reconstructed_train_predictions(self, model_name: str) -> Dict[str, np.ndarray]:
        ...
        return self.predictions.get_reconstructed_train_predictions(model_name)
    ...
    def clear_results(self, model: Optional[str] = None) -> None:
        ...
        self.predictions.clear_results(model)

    def get_results_summary(self) -> Dict[str, Any]:
        """Get a summary of stored results."""
        return self.predictions.get_results_summary()
    ...
    def get_sample_indices_by_class(self,
        ...
        if not sample_ids:
            return {}        # Get targets for these samples
        try:
