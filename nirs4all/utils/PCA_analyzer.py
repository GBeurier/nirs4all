...
class PreprocPCAEvaluator:
    ...
    def plot_cross_dataset_network(self, preproc=None, threshold=None, figsize=(12, 10)):
        """
        Create a network visualization showing which datasets are close together.
        Edge thickness represents similarity (thicker = more similar/closer).

        Args:
            preproc: Specific preprocessing to visualize (None = raw data comparison)
            threshold: Only show edges below this distance (auto if None)
        """
        if self.cross_dataset_df_ is None or self.cross_dataset_df_.empty:
            print("⚠️  No cross-dataset analysis available. Need multiple datasets.")
            return None
        ...
        if preproc is None:
            # Use raw distances
            df = self.cross_dataset_df_[['dataset_1', 'dataset_2', 'centroid_dist_raw', 'spread_dist_raw']].drop_duplicates()
            metric_col = 'centroid_dist_raw'
            title_suffix = 'Raw Data'
        else:
            df = self.cross_dataset_df_[self.cross_dataset_df_['preproc'] == preproc]
            metric_col = 'centroid_dist_pp'
            pp_display = preproc.split('|')[-1].replace('MinMax>', '').replace('>', '→')
            title_suffix = f'{pp_display}'

        if df.empty:
            print(f"⚠️  No data for preprocessing: {preproc}")
            return None
        ...
        # Add nodes
        datasets = set(df['dataset_1']).union(set(df['dataset_2']))
        for ds in datasets:
            G.add_node(ds)

        # Add edges (convert distance to similarity for edge weights)
        distances = df[metric_col].values
        ...
        for _, row in df.iterrows():
            ...
            if dist <= threshold:
                ...
                G.add_edge(row['dataset_1'], row['dataset_2'],
                          weight=similarity, distance=dist)

        if len(G.edges()) == 0:
            print(f"⚠️  No edges below threshold {threshold:.2f}. Try increasing threshold.")
            return None
        ...
        ax.set_title(f'Cross-Dataset Proximity Network\n{title_suffix}\n' +
                    f'(Edges shown: distance ≤ {threshold:.2f}, thickness ∝ similarity)',
                    fontsize=13, fontweight='bold', pad=20)
        ax.axis('off')
        plt.tight_layout()
        return fig
