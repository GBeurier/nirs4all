...
def main():
    ...
    pipeline = [
        ...
        {"feature_augmentation": [Gaussian(), SavitzkyGolay(), StandardNormalVariate(), Haar()]},  # No augmentation for simplicity
    ...
    # Best train model
    best_entry = run_predictions.top_k(1)[0]
    ...
    prediction_dataset = DatasetConfigs(['sample_data/regression'])

    print("\n--- Method 1: Entry ---")
    predictions1, _ = PipelineRunner.predict(best_entry, prediction_dataset, verbose=0)
    preds1 = predictions1.top_k(1)[0]['y_pred'].flatten()
    print(f"First: {preds1[0]:.6f}, Last: {preds1[-1]:.6f}")

    print("\n--- Method 2: ID ---")
    predictions2, _ = PipelineRunner.predict(prediction_id, prediction_dataset, verbose=0)
    preds2 = predictions2.top_k(1)[0]['y_pred'].flatten()
    print(f"First: {preds2[0]:.6f}, Last: {preds2[-1]:.6f}")

    print("\n--- Method 3: Path ---")
    predictions3, _ = PipelineRunner.predict(f"results/{config_path}", prediction_dataset, verbose=0)
    preds3 = predictions3.top_k(1)[0]['y_pred'].flatten()
