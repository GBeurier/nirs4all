...
from tests.test_data_generator import TestDataManager
...
class TestNirs4allIntegration:
    ...
    def test_data_shape_validation(self, test_data_manager):
        ...
        X_df, _, _ = load_csv(str(Path(dataset_folder) / "Xcal.csv.gz"), delimiter=';', has_header=False)
        y_df, _, _ = load_csv(str(Path(dataset_folder) / "Ycal.csv.gz"), delimiter=';', has_header=False)
