...
class TestExecutionContext:
    ...
    def test_dict_like_getitem(self):
        ...
        assert context["partition"] == "train"
        assert context["y"] == "encoded"

    def test_dict_like_setitem_known_fields(self):
        """Test dict-like __setitem__ for known fields."""
        context = ExecutionContext()

        context["partition"] = "test"
        context["y"] = "encoded"
        context["keyword"] = "model"

        assert context.selector.partition == "test"
        assert context.state.y_processing == "encoded"
        assert context.metadata.keyword == "model"

    def test_dict_like_setitem_custom_fields(self):
        """Test dict-like __setitem__ for custom fields."""
        context = ExecutionContext()

        context["custom_key"] = "custom_value"

        assert context.custom["custom_key"] == "custom_value"
        assert context["custom_key"] == "custom_value"

    def test_dict_like_get_with_default(self):
        """Test dict-like get with default value."""
        context = ExecutionContext()

        assert context.get("partition") == "all"
        assert context.get("nonexistent", "default") == "default"

    def test_dict_like_contains(self):
        """Test dict-like membership test."""
        context = ExecutionContext(
            selector=DataSelector(partition="train")
        )

        assert "partition" in context
        assert "y" in context
        assert "nonexistent" not in context

    def test_dict_like_keys(self):
    ...
    def test_dict_like_values(self):
        """Test dict-like values() method."""
        context = ExecutionContext(
            selector=DataSelector(partition="train")
        )
    ...
    def test_dict_like_items(self):
        ...
        assert items["partition"] == "train"
        assert items["y"] == "encoded"
