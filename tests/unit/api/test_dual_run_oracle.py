...
def test_dual_comparison_bounds_the_mismatch_ledger() -> None:
    ...
    native_rows = [{**legacy_rows[0], "y_pred": np.ones(300)}]
    legacy = _Result(num_predictions=1, best_score=0.3, best_rmse=0.3, cv_best_score=0.3, validation_rows=legacy_rows)
    native = _Result(num_predictions=1, best_score=0.3, best_rmse=0.3, cv_best_score=0.3, validation_rows=native_rows)

    report = _dual_comparison_report(legacy, native, legacy_seconds=2.0, native_seconds=1.0)  # type: ignore[arg-type]

    assert len(report["mismatches"]) == 257
    assert report["mismatches"][-1] == {"field": "mismatches", "reason": "truncated", "retained": 256}
...
def test_dual_tolerance_ledger_size_is_bounded_before_json_decode(monkeypatch: pytest.MonkeyPatch) -> None:
    run_module = importlib.import_module("nirs4all.api.run")

    class _OversizedResource:
        def joinpath(self, _name: str) -> _OversizedResource:
            return self

        def read_bytes(self) -> bytes:
            return b"x" * (run_module._DUAL_MAX_LEDGER_BYTES + 1)

    monkeypatch.setattr(run_module.importlib.resources, "files", lambda _package: _OversizedResource())

    with pytest.raises(DualRunUnsupported, match="compatibility ledger larger"):
...
def test_dual_returns_the_native_result_after_temporary_legacy_comparison(monkeypatch: pytest.MonkeyPatch) -> None:
    run_module = importlib.import_module("nirs4all.api.run")
    import nirs4all.pipeline.dagml.run_backend as run_backend
    ...
    report = {"schema_version": 3, "engine": "dual", "mismatches": []}
    monkeypatch.setattr(run_backend, "run_via_dagml", lambda *_args, **_kwargs: native)
    monkeypatch.setattr(run_module, "_dual_comparison_report", lambda *_args, **_kwargs: report)
    pipeline, dataset = _supported_request()
    ...
    assert result._dual_run_report is report
