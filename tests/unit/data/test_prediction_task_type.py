...
class TestTopWithTaskType:
    ...
    def test_top_regression_only(self, mixed_predictions):
        results = mixed_predictions.top(5, rank_metric="rmse", rank_partition="val", task_type="regression")
        assert len(results) == 2
        assert all(r["task_type"] == "regression" for r in results)

    def test_top_classification_only(self, mixed_predictions):
        results = mixed_predictions.top(5, rank_metric="balanced_accuracy", rank_partition="val", task_type="classification")
        assert len(results) == 2
        assert all("classification" in r["task_type"] for r in results)

    def test_top_clf_alias(self, mixed_predictions):
        results = mixed_predictions.top(5, rank_metric="balanced_accuracy", rank_partition="val", task_type="clf")
        assert len(results) == 2
        assert all("classification" in r["task_type"] for r in results)

    def test_top_reg_alias(self, mixed_predictions):
        results = mixed_predictions.top(5, rank_metric="rmse", rank_partition="val", task_type="reg")
        assert len(results) == 2
        assert all(r["task_type"] == "regression" for r in results)

    def test_top_binary_alias(self, mixed_predictions):
        results = mixed_predictions.top(5, rank_metric="balanced_accuracy", rank_partition="val", task_type="binary")
        assert len(results) == 2
    ...
    def test_top_no_filter_returns_all(self, mixed_predictions):
        ...
        results = mixed_predictions.top(10, rank_metric="rmse", rank_partition="val")
    ...
    def test_top_mixed_warns(self, mixed_predictions):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            mixed_predictions.top(5, rank_partition="val")
...
class TestGetBestWithTaskType:
    ...
    def test_get_best_regression(self, mixed_predictions):
        best = mixed_predictions.get_best(metric="rmse", task_type="regression")
        assert best is not None
        assert best["task_type"] == "regression"

    def test_get_best_classification(self, mixed_predictions):
        best = mixed_predictions.get_best(metric="balanced_accuracy", task_type="classification")
        assert best is not None
