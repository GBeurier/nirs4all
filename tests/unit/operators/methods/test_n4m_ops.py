...
# Keep the proof surface self-contained: release CI installs nirs4all-methods as
# a wheel and must not depend on a developer's sibling source checkout.
_METHODS_FIXTURES = Path(__file__).resolve().parents[3] / "fixtures" / "methods"
...
class TestSNVFixtureParity:
    def test_snv_matches_methods_golden_fixture(self, methods_available: bool) -> None:
        ...
        for case in fixture["cases"]:
            ...
            # The repository-local golden was generated by the explicit NumPy
            # row-wise SNV reference; native matches it to floating precision.
